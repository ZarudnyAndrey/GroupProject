...
public class AccountControllerIntegrationTest {
  ...
  @Test
  void registration() {
    HttpEntity<RegistrationApi> entity = new HttpEntity<>(registrationApi);
    ...
    Assertions.assertEquals("User successfully added", response.getBody().getError());
  ...
  void changePassword() {
    ...
    Assertions.assertEquals(HttpStatus.OK, response.getStatusCode());
    Assertions
        .assertEquals("Your password was successfully changed", response.getBody().getError());
    Assertions.assertEquals("Ok", response.getBody().getData().getMessage());
  }

  private ResponseEntity<ResponseApi<? extends ReportApi>> getExchangePassword(String url,
      HttpMethod httpMethod, HttpEntity<SetPasswordApi> entity) {
  ...
  private ResponseEntity<ResponseApi<? extends ReportApi>> getExchangeEmail(String url,
      HttpMethod httpMethod, HttpEntity<RequestNewEmailApi> entity) {
  ...
  private ResponseEntity getExchangeRegistration(String url, HttpMethod httpMethod,
      HttpEntity<RegistrationApi> entity) {
    return restTemplate
        .exchange(url, httpMethod, entity,
            new ParameterizedTypeReference<ResponseApi<? extends ReportApi>>() {
              @Override
